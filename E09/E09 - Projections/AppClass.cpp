...
void Application::Display(void)
{
	...
	case 2:
		m_pCamera->ResetCamera();
		m_pCamera->SetPerspective(false); // change to orthographic
		break;
	case 3:
		m_pCamera->ResetCamera();

		m_pCamera->SetPosition(vector3(25.0f, 0.0f, 0.0f)); // have cone point left
		m_pCamera->SetTarget(vector3(0.0f, 0.0f, 1.0f)); 
		m_pCamera->SetUp(vector3(0.0f, 0.0f, -1.0f)); // flip camera
		break;
	case 4:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(vector3(0.0f, 0.0f, -15.0f)); // move camera to other side
		break;
	case 5:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(vector3(0.0f, 0.0f, -15.0f)); // move camera to other side

		m_pCamera->SetNearFar(vector2(5.0f, 60.0f)); // change near and far to hide triangle
		break;
	case 6:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(vector3(0.0f, 0.0f, -15.0f)); // move camera to other side

		m_pCamera->SetNearFar(vector2(0.1f, 10.0f)); // change near and far to hide hexagon
		break;
	case 7:
		m_pCamera->ResetCamera();
		m_pCamera->SetUp(vector3(0.0f, -1.0f, 0.0f)); // flip camera
		break;
	}
